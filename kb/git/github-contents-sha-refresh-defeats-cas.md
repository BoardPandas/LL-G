...
- **A warning is not a guard here -- the tool has to refuse.** The obvious fix, once you know the refresh is wrong, is to detect the missing-SHA case and warn. That does not work: the PUT still goes through, returns 200, and the discarded change leaves no trace on either side, so the only evidence is a line on stderr in a session already moving to its next step. Make an existing path with no base-SHA a non-zero exit and print the read-the-SHA-first recipe in the error text. Resist an override flag -- the remedy is one extra command, and a flag is pasted in every time the refusal fires, which restores exactly the behaviour being removed. Creating a NEW path is the one case that legitimately needs no SHA, because a 404 means there is nothing to lose.
