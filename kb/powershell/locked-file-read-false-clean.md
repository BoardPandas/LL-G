...
- **`FileShare::ReadWrite` is not a skeleton key — it only works if the *writer* opened the file with sharing.** SQLite-WAL stores (Windows toast `wpndatabase.db`, Chromium `History`) share and are readable while in use; Chromium's live **`Sessions\Tabs_*` / `Session_*`** files are held exclusively and your open still throws `The process cannot access the file ... because it is being used by another process`. When that happens the positive control returns 0 and you MUST report INCONCLUSIVE, not CLEAN — exactly the failure this entry is about. To inspect open browser tabs, read the History DB (+ `-wal`) instead, or have the user act on the live window.
- To query a Chromium `History`/`Cookies` SQLite DB when `sqlite3.exe` is absent (it is not shipped with Windows), copy the file and P/Invoke **`C:\Windows\System32\winsqlite3.dll`** (present on Win10/11) via `Add-Type`: `sqlite3_open_v2` / `prepare_v2` / `step` / `column_text`. That yields real `visit_time` timestamps — raw-byte regex over the file gives you URLs but no times, which is usually the thing that decides the case.
- Related: [Error handling with -ErrorAction Stop](error-handling.md), [Don't assume catch-block operations will succeed](catch-block-safety.md), [ConvertFrom-Json is case-insensitive](convertfrom-json-case-insensitive-keys.md).
